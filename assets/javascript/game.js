...
//random selection from possible answers
        random = Math.floor(Math.random() * puzzleAnswers.length);
...
//these arrays hold all the stuff to check guesses against and update the display
        for (var i = 0; i < howLongIspuzzleItem; i++) {
...
//make sure choice is a letter and not chosen before
            var livesRemaining, backup, backup;
...
//close 1st condition
                    ...
                    var backup = [];
                    for (var i = 0; i < puzzleUnderscores.length; i++) {
